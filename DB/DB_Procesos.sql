...
-- DECLARE VAL_AUTO INTEGER DEFAULT 2;
	
	-- SET VAL_AUTO = (SELECT IFNULL( MAX(moodle_course_id)+1 , 2 ) FROM am_cursos);
	-- ALTER TABLE mdl_course AUTO_INCREMENT = VAL_AUTO;
	...
	FROM mdl_enrol a, am_temp_cursos b WHERE a.courseid = b.id_course AND a.enrol = 'db') AS T2 
...
BEGIN
	...
	UNIX_TIMESTAMP(now()),
	a.per_aca, 
	CONCAT(b.codigo_curso, "_", a.grupo), 1
	...
	SELECT b.id_moodle, c.id, UNIX_TIMESTAMP(now())
	FROM am_usuario_curso a, am_usuario b, mdl_groups c
	WHERE     a.codigo_curso = c.idnumber
	AND a.grupo = c.name
