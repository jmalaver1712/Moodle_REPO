...
class interfaz extends Conexion{
	...
	public function datos_usuario(){
		...
		$result = $this->_db->query("SELECT a.roleid FROM ".DB_PRE."role_assignments a, ".DB_PRE."context b 
	...
	// NOTIFICACIONES
	public function data_noticias(){
		$data = array();

		$hoy = date("Y-m-d");
		$result = $this->_db->query("SELECT id, titular, imagen, enlace, resumen, fecha_publicacion, fecha_final FROM am_notificaciones WHERE (fecha_publicacion <= '$hoy' AND fecha_final >= $hoy) ORDER BY fecha_publicacion DESC LIMIT 0,4");
		if (!$result) {
	...
	public function lista_cursos(){
        ...
        $data_user = $this->datos_usuario();
        $result = $this->_db->query("
			SELECT a.id, a.wwwroot, d.nombre_curso, d.moodle_course_id
			FROM ".DB_PRE."mnet_host a, am_usuario_curso b, am_numero_campus c, am_cursos d, am_campus e
